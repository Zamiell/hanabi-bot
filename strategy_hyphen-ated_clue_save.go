...
func (d *Hyphenated) CheckNextPlayerSave(g *Game) *Action {
	...
	if c.IsCritical(g) {
		logger.Debug(c.Name() + " on " + np.Name + "'s chop is critical.")
	} else if hc.IsPlayableUnique(g, d) {
		logger.Debug(c.Name() + " on " + np.Name + "'s chop is playable and unique.")
	} else if hc.Needs2Save(g, d) {
		logger.Debug(c.Name() + " on " + np.Name + "'s chop needs a 2 Save.")
	}
